...
/**
 * @class SortingNode
 * @brief The main class for the sorting node.
 *
 * This node listens for spatial detections from a depthai camera, finds an object
 * within a predefined workspace in the robot's `fr3_link0` frame, and broadcasts its
 * position as a transform from the `camera_link` frame to a new `object_link` frame.
 */
class SortingNode : public rclcpp::Node
{
public:
    /**
     * @brief The calculated width of the detected object in meters.
     * This value is updated whenever a new object is detected and its frame is published.
     */
...
private:
    ...
    /**
     * @brief Subscription to the spatial detection topic.
     */
    ...
    /**
     * @brief Subscription to the camera info topic.
     */
    ...
    /**
     * @brief Flag to indicate if camera intrinsics have been received.
     */
    bool camera_intrinsics_received_ = false;
    /**
     * @brief Flag to control whether the detection callback processes messages.
     */
    bool detection_enabled_ = false;
    /**
     * @brief Stores the latest point cloud message.
     */
    sensor_msgs::msg::PointCloud2::ConstSharedPtr latest_point_cloud_;
    /**
     * @brief Stores the camera intrinsic parameters.
     */
    ...
    /**
     * @brief TF2 buffer to store transforms.
     */
